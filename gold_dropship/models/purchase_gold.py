...
class PurchaseGold(models.Model):
    ...
    price_unit_iqd = fields.Monetary(compute="_compute_price_iqd",store=True,currency_field="currency_id_iqd",string="Price Unit")
    ...
    total_amount_excl_iqd = fields.Monetary(compute="_compute_total", aggregator='sum',store=True,string="Amount without tax",digits=(16, 3),currency_field="currency_id_iqd")
    ...
    all_price_unit_iqd = fields.Monetary(compute="_compute_total_data",currency_field="currency_id_iqd",store=True)
    ...
    def _compute_total_data(self):
        for rec in self:
            rec.all_price_unit_iqd = sum([i.price_unit_iqd for i in rec.po_gold_line])
