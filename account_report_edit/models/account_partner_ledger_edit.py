...
class PartnerLedgerCustomInherit(models.AbstractModel):
    _inherit = 'account.partner.ledger.report.handler'

    @api.model
    def _currency_table_apply_rate_custome(self, value: SQL) -> SQL:
        ...
        """
        return SQL("(%(value)s) * COALESCE(1, 1)", value=value)
    ...
    def _build_partner_lines(self, report, options, level_shift=0):
        ...
        totals_by_column_group = {
            column_group_key: {
                total: 0.0
                for total in ['debit', 'credit', 'amount', 'balance', 'balance_gold', 'debit_gold', 'credit_gold']
            }
        ...
        for partner, results in partners_results:
            ...
            for column_group_key in options['column_groups']:
                ...
                # Gold Fields
                partner_values[column_group_key]['debit_gold'] = partner_sum.get('debit_gold', 0.0)
                partner_values[column_group_key]['credit_gold'] = partner_sum.get('credit_gold', 0.0)
                partner_values[column_group_key]['balance_gold'] = partner_sum.get('balance_gold', 0.0)
                ...
                # Gold Fields
                totals_by_column_group[column_group_key]['debit_gold'] += partner_values[column_group_key]['debit_gold']
                totals_by_column_group[column_group_key]['credit_gold'] += partner_values[column_group_key][
                    'credit_gold']
                totals_by_column_group[column_group_key]['balance_gold'] += partner_values[column_group_key][
                    'balance_gold']
    ...
    def _query_partners(self, report, options):
        ...
        def assign_sum(row):
            fields_to_assign = ['balance', 'debit', 'credit', 'amount', 'balance_gold', 'credit_gold', 'debit_gold']
        ...
        self._cr.execute(query)
        totals = {}
        for total_field in ['debit', 'credit', 'amount', 'balance', 'balance_gold', 'credit_gold', 'debit_gold']:
        ...
        for row in self._cr.dictfetchall():
            ...
            # Gold Fields
            totals['debit_gold'][row['column_group_key']] += row['debit_gold']
            totals['credit_gold'][row['column_group_key']] += row['credit_gold']
            totals['balance_gold'][row['column_group_key']] += row['balance_gold']
        ...
        if None in groupby_partners:
            ...
            for column_group_key in options['column_groups']:
                ...
                # Gold Fields
                groupby_partners[None][column_group_key]['debit_gold'] += totals['credit_gold'][column_group_key]
                groupby_partners[None][column_group_key]['credit_gold'] += totals['debit_gold'][column_group_key]
                groupby_partners[None][column_group_key]['balance_gold'] -= totals['balance_gold'][column_group_key]
    ...
    def _get_sums_without_partner(self, options):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(SQL(
                ...
                # debit_gold_select=self._currency_table_apply_rate_custome(
                #     SQL("CASE WHEN aml_with_partner.balance_gold > 0 THEN 0 ELSE partial.amount END")),
                # credit_gold_select=self._currency_table_apply_rate_custome(
                #     SQL("CASE WHEN aml_with_partner.balance_gold < 0 THEN 0 ELSE partial.amount END")),
                # balance_gold_select=self._currency_table_apply_rate_custome(
                #     SQL("-SIGN(aml_with_partner.balance_gold) * partial.amount")),
                table_references=query.from_clause,
                currency_table_join=report._currency_table_aml_join(column_group_options,
    ...
    def _report_expand_unfoldable_line_partner_ledger(self, line_dict_id, groupby, options, progress, offset,
        ...
        for result in aml_results:
            if options[
                'export_mode'] != 'print' and report.load_more_limit and treated_results_count == report.load_more_limit:
                ...
                if None in rslt:
                    rslt[None].append({
                        **aml_result,
                        ...
                        'debit_gold': aml_result['credit_gold'],
                        'credit_gold': aml_result['debit_gold'],
                        'balance_gold': -aml_result['balance_gold'],
                    })
            else:
                rslt[aml_result['partner_id']].append(aml_result)
    ...
    def _get_report_line_move_line(self, options, aml_query_result, partner_line_id, init_bal_by_col_group,
                                   init_amount_by_col_group, level_shift=0):
            ...
            if col_value is None:
                ...
                if col_expr_label == 'amount_currency':
                    ...
                    if currency == self.env.company.currency_id:
                        col_value = ''
                if col_expr_label == 'balance_gold' and init_amount_by_col_group:
                    col_value += init_amount_by_col_group[column['column_group_key']]
